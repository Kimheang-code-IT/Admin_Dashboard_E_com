  ...
  <div :class="['mx-auto transition-all duration-300', isSidebarCollapsed ? 'max-w-full px-2' : 'max-w-7xl']">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-2 mb-2">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-sm shadow p-3 transition-colors duration-300">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-2">{{ t('avgOrder') }} ${{ avgOrder.toFixed(2) }}</p>
          </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-sm shadow p-3 transition-colors duration-300">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-sm text-gray-600 dark:text-gray-300 mt-1">{{ t('orders') }}</p>
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-2">{{ t('itemsSold') }} {{ totalItemsSold }}</p>
          </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-sm shadow p-3 transition-colors duration-300">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-3xl font-bold text-gray-800 dark:text-white">${{ bestSale.total ? bestSale.total.toFixed(2) :
              '0.00' }}</p>
            <p class="text-sm text-gray-600 dark:text-gray-300 mt-1">{{ t('bestSale') }}</p>
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-2">{{ t('saleId') }} {{ bestSale.id }}</p>
          </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-sm shadow p-3 transition-colors duration-300">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-sm text-gray-600 dark:text-gray-300 mt-1">{{ t('reportingRange') }}</p>
          </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-sm shadow  transition-colors duration-300">
      ...
      <div class="p-2">
        <div class="flex flex-col md:flex-row gap-4 items-center justify-between">
          <div class="relative w-[400px]">
            ...
            <input v-model="searchQuery" type="text" :placeholder="t('searchBySaleId')"
      ...
      <div
        :class="['overflow-y-auto border border-gray-200 dark:border-gray-700 rounded-sm transition-all duration-300', uiStore.isFullscreen ? 'max-h-[590px]' : 'max-h-[455px]']">
            ...
            <tr v-for="(sale, index) in filteredSales" :key="sale.id"
              ...
              <td class="px-4 py-3 text-sm">
                <div>
                  ...
                  <p class="text-xs text-gray-500 dark:text-gray-400">{{ sale.date }}, {{ sale.time }}</p>
                </div>
              </td>
              <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300">{{ sale.products }}</td>
              <td class="px-4 py-3 whitespace-nowrap text-sm font-bold text-gray-900 dark:text-white">{{ sale.itemsSold
                }} {{ t('pcs') }}</td>
              ...
              <td class="px-4 py-3 whitespace-nowrap text-sm font-bold text-gray-900 dark:text-white">${{
                sale.avgPerItem.toFixed(2) }}</td>
              <td class="px-4 py-3 whitespace-nowrap text-sm font-bold text-gray-900 dark:text-white">${{
                sale.total.toFixed(2) }}</td>
    ...
    <!-- Sale Details Drawer -->
    <div v-if="showDrawer" class="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-end transition-opacity"
      @click.self="closeDrawer">
      <div class="bg-white w-[500px] h-full shadow-2xl overflow-y-auto transform transition-transform">
        <!-- Drawer Header -->
        <div
          class="sticky top-0 bg-white text-black px-6 py-5 flex items-center justify-between z-10  border-b border-gray-200">
          <div class="flex items-center gap-3">
            ...
            <h2 class="text-xl font-bold text-black">{{ t('saleDetails') }}</h2>
          </div>
          <button @click="closeDrawer" class="text-black hover:bg-gray-100 rounded-sm p-2 transition-colors">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-black" fill="none" viewBox="0 0 24 24"
              stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
            </svg>
        ...
        <div class="p-6" v-if="selectedSale">
          ...
          <div class="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-5 mb-6 border border-blue-100">
            <div class="flex items-center justify-between mb-4">
              <div class="flex items-center gap-3">
                ...
                <div>
                  <p class="text-sm text-gray-600 font-medium">{{ t('no') }}</p>
                  <p class="text-lg font-bold text-gray-900">{{ selectedSale.id }}</p>
                </div>
              </div>
              <div class="text-right">
                <p class="text-sm text-gray-600 font-medium">{{ t('totalAmount') }}</p>
          ...
          <div class="space-y-4 mb-6">
            <!-- Created Date/Time -->
            <div class="bg-white rounded-sm border border-gray-200 p-4 hover:shadow-md transition-shadow">
              <div class="flex items-center gap-3">
                <div class="w-10 h-10 bg-gray-100 rounded-sm flex items-center justify-center flex-shrink-0">
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <p class="text-sm font-semibold text-gray-900">{{ selectedSale.date }}, {{ selectedSale.time }}</p>
                </div>
              </div>
            </div>
            ...
            <div class="bg-white rounded-sm border border-gray-200 p-4 hover:shadow-md transition-shadow">
              <div class="flex items-center gap-3">
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <p class="text-sm font-semibold text-gray-900">{{ selectedSale.itemsSold }} {{ t('pcs') }}</p>
                </div>
              </div>
            </div>
            ...
            <div class="bg-white rounded-sm border border-gray-200 p-4 hover:shadow-md transition-shadow">
              <div class="flex items-center gap-3">
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <p class="text-sm font-semibold text-gray-900">{{ selectedSale.soldBy || t('admin') }}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="bg-white rounded-sm border border-gray-200 overflow-hidden">
            ...
            <div class="divide-y divide-gray-200">
              <div v-for="(product, index) in selectedSale.topProducts" :key="index"
                ...
                <div class="flex items-center gap-3">
                  ...
                  <p class="text-sm font-medium text-gray-900 flex-1">{{ product }}</p>
                </div>
              </div>
            </div>
          </div>
...
const reportingRange = computed(() => {
  if (filterDateFrom.value && filterDateTo.value) {
    return `${filterDateFrom.value} ${t('to')} ${filterDateTo.value}`
  } else if (filterDateFrom.value) {
    return `${t('from')} ${filterDateFrom.value}`
  } else if (filterDateTo.value) {
    return `${t('to')} ${filterDateTo.value}`
  }
  return t('allTime')
})
  ...
  if (searchQuery.value) {
    ...
    filtered = filtered.filter(sale =>
      sale && sale.id && (
        ...
        (sale.topProducts && sale.topProducts.some(p => p && p.toLowerCase().includes(query)))
      )
    )
  }
...
const totalRevenue = computed(() => {
  if (!filteredSales.value || filteredSales.value.length === 0) return 0
  return filteredSales.value.reduce((sum, sale) => sum + (sale?.total || 0), 0)
})

const totalOrders = computed(() => filteredSales.value?.length || 0)

const totalItemsSold = computed(() => {
  if (!filteredSales.value || filteredSales.value.length === 0) return 0
  return filteredSales.value.reduce((sum, sale) => sum + (sale?.itemsSold || 0), 0)
})
...
const bestSale = computed(() => {
  if (!filteredSales.value || filteredSales.value.length === 0) {
    return { id: 'N/A', total: 0 }
  }
  const sorted = [...filteredSales.value].sort((a, b) => (b?.total || 0) - (a?.total || 0))
