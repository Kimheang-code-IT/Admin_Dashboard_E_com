<template>
  <div :class="['mx-auto transition-all duration-300', isSidebarCollapsed ? 'max-w-full px-2' : 'max-w-7xl']">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-sm shadow p-3 transition-colors duration-300">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-sm text-gray-600 dark:text-gray-300 mt-1">{{ t('totalStockUnits') }}</p>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-sm shadow p-3 transition-colors duration-300">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-sm text-gray-600 dark:text-gray-300 mt-1">{{ t('categories') }}</p>
          </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-sm shadow transition-colors duration-300">
        ...
        <div class="flex flex-col md:flex-row gap-4 items-center justify-between">
          ...
          <div class="flex items-center gap-4">
            ...
            <button @click="openAddDrawer"
              class="px-6 py-2 bg-blue-600 text-white text-[13px] rounded-sm hover:bg-blue-700 transition-colors font-medium flex items-center gap-2">
      ...
      <div
        :class="['overflow-y-auto border border-gray-200 dark:border-gray-700 rounded-sm transition-all duration-300', uiStore.isFullscreen ? 'max-h-[593px]' : 'max-h-[459px]']">
        <table class="w-full">
          <!-- Sticky Header -->
            ...
            <tr v-for="(product, index) in filteredProducts" :key="product.id"
              ...
              <td class="px-4 py-3 text-sm">
                <div class="flex items-center gap-2">
                  <div
                    :class="['w-10 h-10 rounded flex items-center justify-center overflow-hidden flex-shrink-0 relative', (!product.image || hasImageFailed(product.id)) ? 'border border-gray-300 bg-blue-100' : '']">
              ...
              <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300">{{ product.totalStock }}
                {{ t('items') }}
              </td>
              ...
              <td class="px-4 py-3 text-center whitespace-nowrap text-sm text-gray-700 dark:text-gray-300">
                <button @click="openAddDamagedDrawer(product)" class="hover:underline cursor-pointer text-red-600"
                  :title="t('addDamaged')">
                  {{ product.damaged }}
                </button>
              </td>
              <td class="px-4 py-3 whitespace-nowrap">
                <span class="px-2 py-1 text-xs font-medium rounded-full"
                  :class="product.status === 'Active' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'">
        ...
        <div
          ...
          <div
            ...
            <div class="flex items-center gap-3">
              <div class="w-10 h-10 bg-gray-100 rounded-sm flex items-center justify-center">
          ...
          <form @submit.prevent="handleSubmit" class="p-6 space-y-4">
            ...
            <div>
              ...
              <div class="relative">
                <label for="imageFile"
                  class="block w-full h-48 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-sm cursor-pointer hover:border-blue-400 dark:hover:border-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors overflow-hidden">
                  <input id="imageFile" ref="imageFileInput" type="file" accept="image/*"
                    @change="handleImageFileSelect" class="hidden" />

                  <!-- Show preview if image exists -->
                  <div v-if="form.image" class="w-full h-full relative flex items-center justify-center bg-gray-50">
            ...
            <div>
              ...
              <input id="inStock" v-model.number="form.inStock" type="number" min="0" required
                class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder:text-[13px] placeholder-gray-500 dark:placeholder-gray-400"
                placeholder="0" />
              <p v-if="errors.inStock" class="mt-1 text-sm text-red-600">{{ errors.inStock }}</p>
            </div>

            <!-- Old Price -->
            <div>
              <label for="oldPrice" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                {{ t('oldPrice') }}
              </label>
              <input id="oldPrice" v-model.number="form.oldPrice" type="number" step="0.01" min="0"
                class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder:text-[13px] placeholder-gray-500 dark:placeholder-gray-400"
                placeholder="0.00" />
              <p v-if="errors.oldPrice" class="mt-1 text-sm text-red-600 dark:text-red-400">{{ errors.oldPrice }}</p>
            </div>
            ...
            <div>
              ...
              <input id="discount" v-model.number="form.discount" type="number" step="0.01" min="0" max="100"
                class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder:text-[13px] placeholder-gray-500 dark:placeholder-gray-400"
                placeholder="0" />
              <p v-if="errors.discount" class="mt-1 text-sm text-red-600 dark:text-red-400">{{ errors.discount }}</p>
            </div>
            ...
            <div class="flex gap-3 pt-4">
              ...
              <button type="button" @click="closeDrawer"
                class="px-4 py-2 bg-gray-200 text-gray-700 rounded-sm hover:bg-gray-300 transition-colors font-medium">
                {{ t('cancel') }}
              </button>
            </div>
          </form>
    ...
    <ConfirmationDialog :show="showAddConfirmDialog" type="info" :title="t('confirmAdd')"
      :message="`${t('confirmAddMessage')} ${t('products')}: ${form.name}`" :confirm-text="t('add')"
      :cancel-text="t('cancel')" @confirm="confirmAdd" @cancel="showAddConfirmDialog = false" />

    <ConfirmationDialog :show="showEditConfirmDialog" type="info" :title="t('confirmUpdate')"
      :message="`${t('confirmUpdateMessage')} ${t('products')}: ${form.name}`" :confirm-text="t('update')"
      :cancel-text="t('cancel')" @confirm="confirmEdit" @cancel="showEditConfirmDialog = false" />

    <ConfirmationDialog :show="showDeleteDialog" type="delete" :title="t('confirmDelete')"
      :message="`${t('confirmDeleteMessage')} ${t('products')}: ${productToDelete?.name || ''}`"
    ...
    <Transition name="drawer-top">
      <div v-if="showStockDrawer" class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-start justify-center "
        ...
        <div class="bg-white dark:bg-gray-800 shadow-xl w-[400px] transition-colors duration-300">
          ...
          <form @submit.prevent="handleStockSubmit" class="p-6 space-y-6">
            ...
            <div>
              ...
              <p class="text-sm text-gray-900 dark:text-white font-medium bg-slate-100 dark:bg-gray-700 p-3 rounded-md">
            ...
            <div>
              ...
              <p v-if="stockForm.errors.quantity" class="mt-1 text-sm text-red-600">{{ stockForm.errors.quantity }}</p>
            </div>
    ...
    <ConfirmationDialog :show="showStockConfirmDialog" type="info"
      :title="stockDrawerType === 'add' ? t('addStock') : t('addDamaged')"
      :message="`${t('areYouSure')} ${stockDrawerType === 'add' ? t('areYouSureAddStock') : t('areYouSureAddDamaged')} ${stockForm.quantity} ${stockForm.quantity === 1 ? t('item') : t('items')}? ${t('products')}: ${selectedProduct?.name || ''}`"
      :confirm-text="t('confirm')" :cancel-text="t('cancel')" @confirm="confirmStockAction"
      @cancel="showStockConfirmDialog = false" />
  </div>
</template>
...
// Use products from store - create local mutable copy for mutations
const products = ref([])
...
// Removed old toast state - using useToast composable now
...
const filteredProducts = computed(() => {
  ...
  if (selectedCategory.value) {
    filtered = filtered.filter(p => p && p.category === selectedCategory.value)
  }
...
const totalStockUnits = computed(() => {
  ...
  return products.value.reduce((sum, p) => sum + (p?.totalStock || 0), 0)
})

const lowStockCount = computed(() => {
  if (!products.value || products.value.length === 0) return 0
  return products.value.filter(p => p && p.stockStatus === 'Low').length
})
...
const topCategory = computed(() => {
  ...
  products.value.forEach(p => {
    if (p && p.category) {
      categoryStock[p.category] = (categoryStock[p.category] || 0) + (p.totalStock || 0)
    }
  })
...
// Toast functions - using useToast composable
// Removed - using useToast composable directly
...
.drawer-top-enter-active .bg-white,
.drawer-top-leave-active .bg-white {
...
.drawer-top-enter-from .bg-white,
.drawer-top-leave-to .bg-white {
